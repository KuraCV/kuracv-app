...
export default function LoginPage() {
  ...
  const { login, loginWithGoogle, authError, clearError, isAuthenticated } = useAuth();

  // Load Google Client ID from runtime config or env var
  useEffect(() => {
    if (typeof window !== "undefined") {
      // Try runtime config first (from config.js in Docker), then env var (for local dev)
      const clientId =
        (window as any).__GOOGLE_CLIENT_ID ||
        process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID ||
        "";
      console.log("[Auth] Google Client ID:", clientId ? "loaded" : "not found");
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    setLocalLoading(true);
    try {
      console.log("[Google Login] Starting with credential...");
      await loginWithGoogle(credentialResponse.credential);
      console.log("[Google Login] Success, should be redirecting...");
      // Navigation is handled inside loginWithGoogle() context call on success
    } catch (err) {
          ...
          <div className="w-full mb-5">
            {googleClientId ? (
              <GoogleLogin
                ...
                onError={() => {
                  // Error handling is done through the callback
                }}
              />
            ) : (
              <button
                disabled
                className="w-full flex items-center justify-center gap-3 bg-surface border border-outline-variant opacity-50 py-3 px-4 rounded-xl font-medium text-label-md text-on-surface cursor-not-allowed"
              >
